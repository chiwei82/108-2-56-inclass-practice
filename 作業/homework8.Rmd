...
list_availableSeats = function(seatingChart){
   ...
   for (x in 1:7) {
     ...
     for(y in 1:9){
       if (temp[[y]]==0){
          count = count+1
         name_of_row = names(seatingChart)[[x]] 
         order = paste0("-",y)
         result = paste0(name_of_row,order)
       output_of_ans32[count] = result
         
       }
     }
   }
   
   return(output_of_ans32)
}
...
update_seatingChart = function(seatingChart, selectedSeat){
  ...
  number_of_minus = str_extract(selectedSeat,"[:digit:]{1}$") %>% as.integer() # 取出selected seat -幾 
  # 指向座位表
  temp <- seatingChart
  temp[[number_of_row]][[number_of_minus]] -> zero_or_one
  # 改值或是顯示錯誤
  
  if (zero_or_one == 0)
  {
    temp[[number_of_row]][[number_of_minus]] = -1
...
remove_seats_covid19 = function(seatingChart , selectedSeat) {
  ...
  number_of_minus = str_extract(selectedSeat,"[:digit:]{1}$") %>% as.integer()
  
  seatingChart[[number_of_row]][[number_of_minus]] = -1
  
  if( number_of_minus-1 > 0 & number_of_minus+1 <= 9 ){
  
  seatingChart[[number_of_row]][(number_of_minus-1)] = -1
  seatingChart[[number_of_row]][(number_of_minus+1)] = -1
  
  } else if ( number_of_minus == 9 ) {
    seatingChart[[number_of_row]][(number_of_minus-1)] = -1
  } else {
    seatingChart[[number_of_row]][(number_of_minus+1)] = -1
  }
...
remove_seats_covid19(seatingChart,"row1-9")
```
