...
```{r ans21}
data228_1 = data228
data228_1[["全宗名編號"]] = factor(str_extract( data228_1[[1]], "[:upper:][:digit:]+[:upper:](?=\\/0)"))

data228_1[["年"]] = as.integer(str_extract(data228[[1]] ,  "(?<=\\/)[:digit:]+(?=\\/)"))
...
請找出101學年入學的經濟系學生在3年級時系上為他規劃的課程科目所有名稱，並把所有可能但不重覆的科目名稱存在econ101junior（class character）
```{r ans32}
